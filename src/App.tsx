...
import { exportCanvasAsZip, shareCanvasToTelegram } from './services/canvasExport';
import './App.css';
...
function App() {
  ...
  const handleShareTelegram = useCallback(async () => {
    ...
    setIsSharing(true);
    try {
      await shareCanvasToTelegram(currentNote);
    } catch (err) {
      console.error('Share to Telegram failed:', err);
    } finally {
      setIsSharing(false);
    }
  }, [currentNote, isSharing]);
      ...
      <header className="toolbar">
        ...
        <div className="toolbar-section tool-buttons">
          ...
          <button
            ...
            disabled={isSharing || currentNote.elements.length === 0}
            title="Share on Telegram"
            style={{ opacity: (isSharing || currentNote.elements.length === 0) ? 0.5 : 1 }}
          >
            <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
